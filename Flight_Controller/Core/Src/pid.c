...
PIDDouble roll;
PIDDouble pitch;
PIDSingle altitude_heading;
PIDSingle altitude_rate;
...
void Single_Alt_Heading_PID_Calculation(PIDSingle* axis, float set_point_altitude, float height/*BMP208 Height*/, float rate/*MPU6050 Acceleration Acc_z*/)
{
	/*********** Single PID Begin (ALT acceleration Position) *************/
	axis->reference = set_point_altitude;	//Set point of alt heading @ alt stick is center.
	axis->meas_value = height;			//Current alt distance @ alt stick is center.

	axis->error = axis->reference - axis->meas_value;	//Define error of alt distance control
	...
	*/
	...
	axis->p_result = axis->error * axis->kp;			//Calculate P result of alt distance control

	axis->error_sum = axis->error_sum + axis->error * DT;	//Define summation of alt distance control
	axis->i_result = axis->error_sum * axis->ki;			//Calculate I result of alt distance control

	axis->error_deriv = -rate;						//Define differentiation of alt distance control
	axis->d_result = axis->error_deriv * axis->kd;	//Calculate D result of alt distance control

	axis->pid_result = axis->p_result + axis->i_result + axis->d_result; //Calculate PID result of alt distance control
	/***************************************************************/
}

void Single_Alt_Rate_PID_Calculation(PIDSingle* axis, float set_point_altitude, float rate/*MPU6050 Acceleration Acc_z*/)
{
	/*********** Single PID Begin (Yaw Angular Rate Control) *************/
	axis->reference = set_point_altitude;	//Set point of alt heading @ yaw stick is not center.
	axis->meas_value = rate;			//Current MPU6050.Acc_z @ alt stick is not center.

	axis->error = axis->reference - axis->meas_value;	//Define error of alt rate control
	axis->p_result = axis->error * axis->kp;			//Calculate P result of alt rate control

	axis->error_sum = axis->error_sum + axis->error * DT;	//Define summation of alt rate control
	axis->i_result = axis->error_sum * axis->ki;			//Calculate I result of alt rate control

	axis->error_deriv = -(axis->meas_value - axis->meas_value_prev) / DT;	//Define differentiation of alt rate control
	axis->meas_value_prev = axis->meas_value;								//Refresh value_prev to the latest value
	axis->d_result = axis->error_deriv * axis->kd;							//Calculate D result of alt rate control

	axis->pid_result = axis->p_result + axis->i_result + axis->d_result; //Calculate PID result of alt control
	/*******************************************************************/
}
